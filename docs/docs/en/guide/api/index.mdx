...
Defines a reactive state with functions to value and set its value, subscribe to changes, and reset it. This type allows for mutable, reactive state management.
...
- **`value`** - Retrieves the computed value.
...
- **`value: QueryData<Data, TError>`** - Retrieves the current query state, including data, loading, success, and error status.
